...
class SteamUdp implements SteamInterface
{
    private  $socket;
     ...
     * @param int $port
     */
    public function __construct(string $path,int $port,int $timeout = 30)
    {
        $errno = null;
        $errnoMessage = null;
        $this->socket = stream_socket_client("udp://{$path}:{$port}",$errno,$errnoMessage,$timeout);
        if (!$this->socket) {
