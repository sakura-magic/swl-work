...
class TagLib {
    ...
    */
     ...
     * 逻辑表达式
    */
    protected array $comparison = [' nheq ' => ' !== ', ' heq ' => ' === ', ' neq ' => ' != ', ' eq ' => ' == ', ' egt ' => ' >= ', ' gt ' => ' > ', ' elt ' => ' <= ', ' lt ' => ' < '];
     ...
     * @throws \Exception
     */
    public function parseTag(string &$content, string $lib = '')
    {
        $tags = [];
    ...
    public function parseAttr(string $str, string $name, string $alias = ''): array
    {
        ...
        if (preg_match_all($regex, $str, $matches)) {
            ...
            if (!empty($this->tags[$name]['expression'])) {
                $offset = strlen($this->tpl->config('taglib_begin_origin') . $name);
                $len = strlen($this->tpl->config('taglib_end_origin'));
                $result['expression'] = substr($str, $offset, -$len);
                // 清除自闭合标签尾部/
